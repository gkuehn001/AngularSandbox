...
namespace AngularSandbox
{
    class Program
    {
        ...
        private static void ProcessRequest(HttpListenerContext ctx)
        {
            ...
            {
                ctx.Response.ContentType = GetMimeType(Path.GetExtension(resourceName));
        ...
        private static string GetTextResource(string resourceName)
        {
            try
            {
                return File.ReadAllText(Path.Combine(_contentDir, resourceName));
            }
            catch
            {
                return null;
        ...
        private static byte[] GetResource(string resourceName)
        {
            try
            {
                using (FileStream fs = new FileStream(
                  Path.Combine(_contentDir, resourceName),
                  FileMode.Open,
                  FileAccess.Read,
                  FileShare.Read))
                {
        ...
        private static void WriteError(HttpListenerContext context, int statusCode)
        {
            try
            {
                string description = Properties.Resources.ResourceManager.GetString("http" + statusCode.ToString());
